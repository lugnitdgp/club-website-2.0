...
// ─── GalleryModal ─────────────────────────────────────────────────────────────
// Uses its own internal visible/closing state with CSS transitions.
// The parent simply mounts/unmounts this component by setting selectedItem.
// NO external AnimatePresence needed — that pattern breaks with createPortal.
...
const GalleryModal = ({ selectedItem, onClose, onSelectItem, mediaItems }: GalleryModalProps) => {
    ...
    const handleClose = useCallback(() => {
        if (closing) return
        setClosing(true)
        // Let CSS transition finish (200ms) then actually unmount
...
const InteractiveBentoGallery: React.FC<InteractiveBentoGalleryProps> = ({
    ...
    return (
        <div className="w-full md:w-3/5 md:mx-auto px-4 py-8">
            ...
            <motion.div
                className="grid grid-cols-1 md:grid-cols-9 gap-3 auto-rows-[150px] md:auto-rows-[60px]"
                initial="hidden"
                animate="visible"
                variants={{
                    hidden: { opacity: 0 },
            ...
            {/* Mount modal only when item selected. GalleryModal handles its own enter/exit CSS transitions. */}
            {selectedItem && (
                <GalleryModal
