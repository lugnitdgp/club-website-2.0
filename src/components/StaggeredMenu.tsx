...
export const StaggeredMenu = forwardRef<{ toggleMenu: () => void; closeMenu: () => void }, StaggeredMenuProps>(
  ...
  const playClose = useCallback(() => {
    ...
    closeTweenRef.current = gsap.to(all, {
      ...
      onComplete: () => {
        ...
        // Reset icon to closed state in case animations were interrupted
        try {
          if (plusHRef.current) gsap.set(plusHRef.current, { rotate: 0, clearProps: 'transform' });
          if (plusVRef.current) gsap.set(plusVRef.current, { rotate: 90, clearProps: 'transform' });
          if (iconRef.current) gsap.set(iconRef.current, { rotate: 0, clearProps: 'transform' });
          spinTweenRef.current?.kill();
        } catch (e) {
          // ignore if refs not available
        }
  ...
  return (
    <div
      className={`sm-scope z-40 ${(isFixed || open) ? 'fixed top-0 left-0 w-screen h-screen overflow-hidden' : 'w-full h-full'}`}
    >
      <div
        className={
      ...
      >
        ...
        >
          ...
          >
            ...
            <span
              ref={iconRef}
              className="sm-icon relative shrink-0 inline-flex flex-col justify-center gap-[5px] w-[22px] h-[18px] [will-change:transform]"
              aria-hidden="true"
            >
              <span
                ref={plusHRef}
                className="block w-full h-[2.5px] rounded-full bg-current [will-change:transform] [transform-origin:center]"
              />
              <span
                className="block w-[65%] h-[2.5px] rounded-full bg-current transition-[opacity,transform] duration-300 ease-in-out"
